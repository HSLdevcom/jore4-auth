...
open class OIDCLoginApiController(
     ...
     */
    override fun login(clientRedirectUrl: String?): ResponseEntity<Void> {
        LOGGER.info("Creating login URI with client redirect URL {}", clientRedirectUrl)

        if (request.session == null) {
            throw UnauthorizedException("Session not found")
        }

        val loginUri = oidcLoginService.createLoginUri(request.session, clientRedirectUrl)
