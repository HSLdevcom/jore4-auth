    ...
    @Operation(
        description =
            "This endpoint is invoked by the OIDC provider and will exchange the given authorization code for an " +
                "access and a refresh token. After a successful code exchange, this endpoint will eventually " +
                "redirect to the configured login page."
    )
    @ApiResponses(
        ApiResponse(responseCode = "302", description = "Redirects to the configured login page."),
        ApiResponse(responseCode = "401", description = "The authenticity of the request cannot be verified.")
    )
    @ResponseStatus(HttpStatus.FOUND)
    @RequestMapping(method = [RequestMethod.GET], value = ["\${api.path.prefix}$EXCHANGE_ENDPOINT_PATH_SUFFIX"])
    fun exchangeCode(
        @Parameter(description = "The authorization code passed on by the OIDC provider.")
        @RequestParam
        code: String,
        @Parameter(
            description =
                "The state to verify the requests authenticity " +
                    "(the state has been generated by us before invoking the OIDC provider's login endpoint)."
        )
        @RequestParam
        state: String,
        session: HttpSession
        ...
        val redirectUri =
            codeExchangeService.exchangeCodeAndRedirect(
